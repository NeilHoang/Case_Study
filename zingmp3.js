...
let info10 = new Info("10","Tướng Quân", "Nhật Phong", "Nhật Phong","Tuong-Quan-Remix-Nhat-Phong-DinhLong.mp3");
...
let info19 = new Info("19","Đi Đu Đưa Đi","Bích Phương","Như bên trái","dudua.mp3");
let info20 = new Info("20","Anh Đợi Em Được Không","Mỹ Tâm","<--","mytam.mp3");
let library = new Music();
library.music.push(info1, info2, info3, info4, info5, info6, info7, info8, info9,
    ...
    for (let i = 0; i < library.music.length; i++) {
        if ((selectMusic.value === library.music[i].stt ) || (library.music[i].name.match(selectMusic.value))) {
            ...
            document.getElementById('demo3').innerHTML = `<audio controls autoplay muted loop style="height: 30px">                                                   
                <source src="${library.music[i].getLinkMusic()}" type="audio/mp3"</audio>`;
